...
(if (and (featurep 'font-lock)
	...
	(tuareg-fontify begin end))
      ...
      (defun tuareg-after-fontify-buffer () ; compatibility with XEmacs 20.x
	(tuareg-fontify (point-min) (point-max)))
	...
	(list
	 (list (string-as-multibyte "\\<\\(external\\|open\\|include\\|rule\\|s\\(ig\\|truct\\)\\|module\\([ \t\n]+type\\)?\\|functor\\|\\(with\\|and\\|let\\)[ \t\n]+\\(type\\|module\\)\\|val\\|type\\|method\\|virtual\\|constraint\\|class\\|in\\|inherit\\|initializer\\|let\\|rec\\|and\\|begin\\|object\\|end\\)\\>")
	       0 'tuareg-font-lock-governing-face nil nil)
	 (list (string-as-multibyte "\\.<\\|>\\.\\|\\.~\\|\\.!")
	       0 'tuareg-font-lock-multistage-face nil nil)
	 (list (string-as-multibyte "\\<\\(as\\|do\\(ne\\|wnto\\)?\\|else\\|for\\|if\\|m\\(atch\\|utable\\)\\|new\\|p\\(arser\\|rivate\\)\\|t\\(hen\\|o\\|ry\\)\\|w\\(h\\(en\\|ile\\)\\|ith\\)\\|lazy\\|exception\\|raise\\|failwith\\|exit\\|assert\\|fun\\(ction\\)?\\)\\>")
	       0 'font-lock-keyword-face nil nil)
	 (list (string-as-multibyte "\\<\\(\\(method\\([ \t\n]+\\(private\\|virtual\\)\\)?\\)\\([ \t\n]+virtual\\)?\\|val\\([ \t\n]+mutable\\)?\\|external\\|and\\|let\\([ \t\n]+rec\\)?\\)\\>[ \t]*\\(\\(\\w\\|[->_ \t,?~.]\\|(\\(\\w\\|[->_ \t,?~.=]\\)*)\\)*\\)")
	       8 'font-lock-variable-name-face nil nil)
	 (list (string-as-multibyte "\\<\\(\\(method\\([ \t\n]+\\(private\\|virtual\\)\\)?\\)\\([ \t\n]+virtual\\)?\\|val\\([ \t\n]+mutable\\)?\\|external\\|and\\|let\\([ \t\n]+rec\\)?\\)\\>[ \t]*\\(['_a-z\277-\377]\\(\\w\\|[._]\\)*\\)\\>[ \t]*\\(\\w\\|[()_?~.]\\|:?\\(\\w\\|[ \t ->_*]\\)*=[ \t]*fun\\(ction\\)?\\>\\)")
	       8 'font-lock-function-name-face t nil)
	 (list (string-as-multibyte "\\<\\(open\\|\\(class\\([ \t\n]+type\\)?\\)\\([ \t\n]+virtual\\)?\\|inherit\\|include\\|module\\([ \t\n]+\\(type\\|rec\\)\\)?\\|type\\)\\>[ \t]*\\(\\(\\<['_A-Za-z\277-\377~?]\\w*\\>[ \t]*\\)*\\<['_A-Za-z\277-\377~?]\\w*\\>:?\\)\\>")
	       7 'font-lock-type-face nil nil)
	 (list (string-as-multibyte "\\<\\([A-Z]\\w*\\>\\)[ \t]*\\.")
	       1 'font-lock-type-face nil nil)
	 (list (string-as-multibyte "\\<\\([?~]?[_A-Za-z\277-\377]\\w*[ \t\n]*:\\)[^:>=]")
	       1 'font-lock-variable-name-face nil nil)
	 (list (string-as-multibyte "\\<exception\\>[ \t]*\\(\\<[_A-Za-z\277-\377]\\w*\\>\\)")
	       1 'font-lock-variable-name-face nil nil)
	 (list (string-as-multibyte "\\<\\(as[lr]\\|false\\|l\\(and\\|xor\\|or\\|s[lr]\\)mod\\|not\\|ref\\|o[fr]\\|true\\|unit\\)\\>")
	       0 'font-lock-constant-face nil nil)
	 (list (string-as-multibyte "[][;,()|{}]\\|[@^!:*=<>&/%+~?---]\\.?\\|\\.\\.\\.*")
	       0 'tuareg-font-lock-operator-face nil nil)
	 (list (string-as-multibyte "^#\\w\\w*\\>")
...
(define-key tuareg-mode-map ")" 'tuareg-electric-rp)
...
(define-key tuareg-mode-map [(control c) (home)] 'tuareg-move-inside-block-opening)
(define-key tuareg-mode-map [(control c) (control down)] 'tuareg-next-phrase)
(define-key tuareg-mode-map [(control c) (control up)] 'tuareg-previous-phrase)
...
(define-key tuareg-mode-map "\C-c?" 'tuareg-interactive-next-error-source)
(define-key tuareg-mode-map "\C-c.c" 'tuareg-insert-class-form)
(define-key tuareg-mode-map "\C-c.b" 'tuareg-insert-begin-form)
(define-key tuareg-mode-map "\C-c.f" 'tuareg-insert-for-form)
(define-key tuareg-mode-map "\C-c.w" 'tuareg-insert-while-form)
(define-key tuareg-mode-map "\C-c.i" 'tuareg-insert-if-form)
(define-key tuareg-mode-map "\C-c.l" 'tuareg-insert-let-form)
(define-key tuareg-mode-map "\C-c.m" 'tuareg-insert-match-form)
(define-key tuareg-mode-map "\C-c.t" 'tuareg-insert-try-form)

(when tuareg-with-caml-mode-p
  ;; caml-types
  (define-key tuareg-mode-map [?\C-c?\C-t] 'caml-types-show-type)
  ;; to prevent misbehavior in case of error during exploration.
  (define-key tuareg-mode-map [(control mouse-2)] 'caml-types-mouse-ignore)
  (define-key tuareg-mode-map [(control down-mouse-2)] 'caml-types-explore)
  ;; caml-help
  (define-key tuareg-mode-map [?\C-c?i] 'ocaml-add-path)
  (define-key tuareg-mode-map [?\C-c?[] 'ocaml-open-module)
  (define-key tuareg-mode-map [?\C-c?]] 'ocaml-close-module)
  (define-key tuareg-mode-map [?\C-c?h] 'caml-help)
  (define-key tuareg-mode-map [?\C-c?\t] 'caml-complete))
  
(defvar tuareg-mode-syntax-table (make-syntax-table)
...
The Font-Lock minor-mode is used accordingly to your customization
...
indentation facilities.
  - Because the `function' keyword has a special indentation (to handle
    case matchs) use the `fun' keyword when no case match is performed.
...
(defconst tuareg-error-chars-regexp
  (string-as-multibyte ".*, .*, [A-\377]+ \\([0-9]+\\)-\\([0-9]+\\):")
...
(defun tuareg-find-=-match ()
  (let ((kwop (tuareg-find-kwop tuareg-find-=-match-regexp
    ...
    (if (string= kwop "and")
	(tuareg-find-and-match)
      (if (and (string= kwop "=") (not (tuareg-false-=-p)))
	  (tuareg-find-=-match))
      kwop)))
...
(defun tuareg-captive-= ()
  (save-excursion
    (tuareg-find-=-match)
    (looking-at "\\<\\(if\\|when\\|module\\|type\\|class\\)\\>")))
	...
	(if (not kwop) (current-column)
	  (cond
	   ...
	   ((and (looking-at "=") (not (tuareg-false-=-p)))
	    (if (or tuareg-lazy-= (tuareg-captive-=)
		...
		(let ((current-column-module-type nil))
		  (+
		   ...
		   (if current-column-module-type
		     ...
		     (current-column))))
	      (let ((kwop (save-excursion (tuareg-find-=-match))))
		(cond
		 ((string= kwop "sig")
		  (tuareg-back-to-paren-or-indentation t)
		  (tuareg-assoc-indent "sig"))
		 ((string= kwop "=")
		  (tuareg-back-to-paren-or-indentation t)
		  (tuareg-find-=-match)
		  (+ (current-column) tuareg-default-indent))
...
(define-key tuareg-interactive-mode-map ")" 'tuareg-electric-rp)
...
(defun tuareg-interactive-next-error-source ()
  ...
  (let ((error-pos) (beg 0) (end 0))
	...
	(message "No syntax or typing error in last phrase.")
      ...
      (add-text-properties beg end '(face tuareg-font-lock-error-face))
      (goto-char beg))))
...
(defconst tuareg-definitions-bind-skip-regexp
  (string-as-multibyte "\\<\\(rec\\|type\\|virtual\\)\\>\\|'[A-Za-z\300-\377][0-9_'A-Za-z\300-\377]*\\|('.*)")
  ...
  (save-excursion
	...
	(if (looking-at tuareg-definitions-regexp)
	    (progn
	      ...
	      (if (looking-at
		   (string-as-multibyte
		    "\\<[A-Za-z\300-\377][0-9_'A-Za-z\300-\377]*\\>"))
